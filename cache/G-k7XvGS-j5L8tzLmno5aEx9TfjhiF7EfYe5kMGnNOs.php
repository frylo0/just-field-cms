...
$__pug_mixins[$__pug_mixin_name](false, array(  ), [[false, 'Open'], [false, './dev.php?page=field&view=tree&path=' . (isset($path) ? $path : null) . '' . $GLOBALS['__jpv_dotWithArrayPrototype_with_ref']($child, 'key') . '&curr_path_i=' . (isset($path_i) ? $path_i : null)]], $__pug_mixin_vars, function ($__pug_children_vars) use (&$__pug_mixins, $__pug_children, $pug_vars, &$pugModule) {
    foreach (array_keys($__pug_children_vars) as $__local_pug_key) {
        if (mb_substr($__local_pug_key, 0, 6) === '__pug_') {
            continue;
        }
