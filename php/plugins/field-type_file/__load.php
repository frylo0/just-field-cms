...
namespace JustField {
   ...
   class T_file {
      ...
      function update(DBItem $item, $value) {
         ...
         $this->type_table_orm->update(['file_src' => '?'])->where("`id_file` = ?")->bind('si', $target_name, $this->id)();
         return $target_file;
      }
      ...
      function duplicate_value_to(DBItem $field, DBItem $new_field) {
         ...
         if (file_exists($file_src)) {
            ...
            $this->type_table_orm->update(['file_src' => '?'])->where("`id_file` = ?")->bind('si', $target_name, $new_field->value_id)();
         }
      ...
      function get_value() {
         ...
         $src = $this->type_table_orm->select('`file_src`')->where("`id_file` = ?")->bind('i', $this->id)()[0]['file_src'];
