...
namespace JustField {
   ...
   class T_video {
      ...
      function update(DBItem $item, $value) {
         ...
         $this->type_table_orm->update(['video_src' => '?'])->where("`id_video` = ?")->bind('si', $target_name, $this->id)();
         return $target_file;
      }
      ...
      function duplicate_value_to(DBItem $field, DBItem $new_field) {
         ...
         if (file_exists($video_src)) {
            ...
            $this->type_table_orm->update(['video_src' => '?'])->where("`id_video` = ?")->bind('si', $target_name, $new_field->value_id)();
         }
      ...
      function get_value() {
         ...
         $src = $this->type_table_orm->select('`video_src`')->where("`id_video` = ?")->bind('i', $this->id)()[0]['video_src'];
